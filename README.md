...
**🤖 每 5 小时更新一批高 Star 开源项目 | AI、深度学习、计算机视觉、AI Agent、计算机专业学习项目精选**
...
**Daily High-Star Recommendations** 是一个自动化开源项目推荐平台。项目通过 GitHub Actions 自动检查，并由脚本按 **每 5 小时时间档**生成推荐内容，抓取并推荐在 **人工智能、深度学习、CNN/Conv、计算机视觉、AIGC 图像检测、AI Agent、MCP、RAG、开发工具、计算机专业基础** 等方向中具有一定热度和活跃度的新项目。

**核心特点：**
- ⭐ **每 5 小时更新内容** - GitHub Actions 定时检查，脚本控制 5 小时时间档
- 🤖 **聚焦 AI 与计算机专业学习方向** - 覆盖深度学习、CNN、计算机视觉、AIGC 检测、Agent、MCP、RAG、算法、系统、数据库、网络安全等领域
...
- 🧭 **分层搜索兜底** - 先搜个人重点学习方向，不够再逐层扩展到计算机专业热门方向和低门槛新项目
- 🧩 **稳定更新记录** - 如果极端情况下没有新项目，也会记录本时间档搜索状态，不重复写入旧项目
...
- 为了尽量保证每次都有新内容，脚本会按阶段扩大搜索池：AI Agent / LLM / MCP / RAG → 深度学习 / CNN / PyTorch → 计算机视觉 / 图像处理 → AIGC 图像检测 / Deepfake / 图像取证 → 算法 / 系统 / 数据库 / 网络安全 → 开发工具 / 自动化 → 多语言工程项目 → 低门槛新项目兜底池。
- 脚本不会再用已经推荐过的热门项目进行兜底。如果 GitHub API 在极端情况下没有返回任何新的未推荐项目，本次仍会写入搜索状态记录，但不会重复推荐旧项目。
...
- 自动搜索高星 AI/ML/深度学习/CV/AIGC 检测/Agent/MCP/计算机专业学习项目
- 使用多个搜索阶段、语言、主题和星标区间提升项目多样性
- 候选项目够 6 个后会提前停止搜索，减少不必要的 API 请求
